...
def get_api_key(provider=None):
    ...
    if provider == "claude":
        ...
        if not k: k = os.getenv("ANTHROPIC_API_KEY", "").strip()
        if not k:
            try: k = st.secrets.get("ANTHROPIC_API_KEY", "").strip()
            except: pass
        return k
    else:
        k = st.session_state.get("openai_api_key", "").strip()
        if not k: k = os.getenv("OPENAI_API_KEY", "").strip()
        if not k:
            try: k = st.secrets.get("OPENAI_API_KEY", "").strip()
            except: pass
        return k
...
def get_client():
    ...
    if provider == "claude":
        return anthropic.Anthropic(api_key=key), "claude"
    else:
...
def ai_complete(system: str, user: str, max_tokens: int = 800, messages: list = None):
    """
    Unified completion call. Returns (text, error).
    Uses multi-turn `messages` list if provided (for chat), 
    otherwise single system+user call.
    """
    provider = get_provider()
    key = get_api_key(provider)
    if not key:
        return None, "❌ No API key set. Please enter your key below."
    ...
    try:
        if provider == "claude":
            ...
            if messages:
                oai_msgs = [{"role": "system", "content": system}] + messages if system else messages
            else:
                oai_msgs = []
                if system: oai_msgs.append({"role": "system", "content": system})
                oai_msgs.append({"role": "user", "content": user})
        ...
        err_str = str(e)
        if "auth" in err_str.lower() or "401" in err_str or "invalid" in err_str.lower() or "incorrect" in err_str.lower():
            return None, f"❌ Invalid {provider.upper()} API key. Get one at {'console.anthropic.com' if provider=='claude' else 'platform.openai.com'}"
        if "rate" in err_str.lower() or "429" in err_str:
            return None, "⏳ Rate limit hit. Wait a moment and try again."
        if "connect" in err_str.lower() or "network" in err_str.lower():
            return None, "🌐 Connection error. Check your internet."
        return None, f"Error: {err_str[:200]}"

def safe_api_call(fn):
    try:
        return fn(), None
    except anthropic.AuthenticationError:
        return None, "❌ Invalid Claude API key."
    except Exception as e:
...
def render_nav():
    ...
    provider_label = "Claude (Anthropic)" if provider == "claude" else "GPT-4o (OpenAI)"
    badge_color = "#a78bfa" if provider == "openai" else "#34d399"
    st.markdown(f"""
...
def render_api_gate():
    st.markdown("""
    <div class="j-card j-card-accent" style="max-width:580px; margin: 2.5rem auto; text-align:center;">
      <div style="font-size:2.5rem; margin-bottom:0.8rem;">🤖</div>
      <div style="font-size:1.1rem; font-weight:600; margin-bottom:0.4rem; color:#e2e8f0;">Connect JARVIS to AI</div>
      <div style="font-size:0.82rem; color:#64748b; margin-bottom:1rem; line-height:1.6;">
        Choose Claude (Anthropic) or GPT-4o (OpenAI) as your AI engine.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        placeholder = "sk-ant-api03-..." if is_claude else "sk-proj-... or sk-..."
        hint = "Get key: console.anthropic.com" if is_claude else "Get key: platform.openai.com/api-keys"
        ...
        st.markdown(f"<div style='font-size:0.72rem; color:#475569; margin-bottom:0.6rem;'>{hint}</div>",
                    unsafe_allow_html=True)
        ...
        if st.button("Initialise JARVIS →", use_container_width=True):
            ...
            if cleaned:
                ...
                st.error(f"Please paste your {'Anthropic' if is_claude else 'OpenAI'} API key.")
...
# If env var / secrets set but flag not yet toggled
if not st.session_state.api_key_set:
    if os.getenv("ANTHROPIC_API_KEY") or (hasattr(st, "secrets") and st.secrets.get("ANTHROPIC_API_KEY","")):
        st.session_state.ai_provider = "claude"
        st.session_state.api_key_set = True
    elif os.getenv("OPENAI_API_KEY") or (hasattr(st, "secrets") and st.secrets.get("OPENAI_API_KEY","")):
        st.session_state.ai_provider = "openai"
        st.session_state.api_key_set = True
