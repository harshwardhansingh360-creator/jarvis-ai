...
def safe_api_call(fn):
    """Wrap a Claude API call and surface clean auth/rate errors."""
    try:
        return fn(), None
    except anthropic.AuthenticationError:
        return None, "❌ Invalid API key. Please check your key in Settings → Secrets (Streamlit Cloud) or re-enter it."
...
Format with clear sections. Keep under 300 words."""
            ...
            t0 = time.time()
            response, err = safe_api_call(lambda: client.messages.create(
...
Be specific and JARVIS-sharp. Max 120 words."""
                t0 = time.time()
                resp, err = safe_api_call(lambda: client.messages.create(
                    model="claude-opus-4-5",
                    max_tokens=300,
                    messages=[{"role": "user", "content": prompt}],
                ))
