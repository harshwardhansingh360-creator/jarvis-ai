...
PROVIDERS = {
    ...
    "openai":     {"label": "GPT-4o (OpenAI) — Paid",            "color": "#f87171", "placeholder": "sk-proj-..."},
}
...
def get_api_key(provider=None):
    ...
    sess_key, env_key = key_map.get(provider, ("api_key", "ANTHROPIC_API_KEY"))
    k = st.session_state.get(sess_key, "").strip()
    if not k:
        k = os.getenv(env_key, "").strip()
    if not k:
        try:
            k = st.secrets.get(env_key, "").strip()
        except Exception:
            pass
    return k

def ai_complete(system: str, user: str, max_tokens: int = 800, messages: list = None):
    """Unified AI completion — works with Gemini, Groq, OpenRouter, Claude, OpenAI."""
    provider = get_provider()
    key = get_api_key(provider)
    if not key:
        return None, f"❌ No API key set for {provider}. Please enter your key."

    # Build full text prompt from system + messages + user
    if messages:
        full_user = "\n\n".join(
    ...
    try:
        ...
        if provider == "gemini":
            ...
            if resp.status_code == 400 and "API_KEY_INVALID" in resp.text:
                return None, "❌ Invalid Gemini API key. Get a free key at aistudio.google.com"
        ...
        # ── GROQ ────────────────────────────────────────────────────────
            ...
            if messages:
                ...
                msgs_payload.append({"role": "user", "content": user})
            resp = requests.post(url, headers={"Authorization": f"Bearer {key}"},
                                 json={"model": "llama-3.1-70b-versatile", "max_tokens": max_tokens,
                                       "messages": msgs_payload}, timeout=30)
            if resp.status_code == 401:
                return None, "❌ Invalid Groq API key. Get a free key at console.groq.com"
            if resp.status_code != 200:
        ...
        # ── OPENROUTER ──────────────────────────────────────────────────
            ...
            resp = requests.post(url,
                headers={"Authorization": f"Bearer {key}", "HTTP-Referer": "https://jarvis-ai.app"},
                json={"model": "mistralai/mistral-7b-instruct:free",
                      "max_tokens": max_tokens, "messages": msgs_payload}, timeout=30)
            if resp.status_code == 401:
                return None, "❌ Invalid OpenRouter key. Get one free at openrouter.ai"
            if resp.status_code != 200:
        ...
        # ── CLAUDE ──────────────────────────────────────────────────────
            ...
            msgs_payload = messages if messages else [{"role": "user", "content": user}]
            payload = {"model": "claude-opus-4-5", "max_tokens": max_tokens, "messages": msgs_payload}
            if system:
            ...
            if resp.status_code == 401:
                return None, "❌ Invalid Claude API key. Get one at console.anthropic.com"
            if resp.status_code != 200:
                return None, f"❌ Claude error {resp.status_code}: {resp.text[:200]}"
        ...
        # ── OPENAI ──────────────────────────────────────────────────────
            ...
            if messages:
                ...
                msgs_payload.append({"role": "user", "content": user})
            resp = requests.post(url, headers={"Authorization": f"Bearer {key}"},
                json={"model": "gpt-4o", "max_tokens": max_tokens, "messages": msgs_payload}, timeout=30)
            if resp.status_code == 401:
                return None, "❌ Invalid OpenAI API key. Get one at platform.openai.com"
            if resp.status_code != 200:
...
def get_client():
    return None, get_provider()  # legacy compat shim
...
def render_nav():
    ...
    provider_label = PROVIDERS.get(provider, PROVIDERS["gemini"])["label"].split(" —")[0]
    badge_color = PROVIDERS.get(provider, PROVIDERS["gemini"])["color"]
    st.markdown(f"""
    <div class="jarvis-nav">
      ...
      <span class="jarvis-status">
        <span class="status-dot"></span> AI Online &nbsp;·&nbsp;
        <span style="color:{badge_color}; font-size:0.72rem; font-weight:600;">{provider_label}</span>
      </span>
    </div>
    """, unsafe_allow_html=True)
...
def render_api_gate():
    ...
    <div style="max-width:620px; margin: 2rem auto;">
      <div class="j-card j-card-accent" style="text-align:center; margin-bottom:1.2rem;">
        <div style="font-size:2.2rem; margin-bottom:0.6rem;">🤖</div>
        <div style="font-size:1.15rem; font-weight:600; color:#e2e8f0; margin-bottom:0.3rem;">Initialise JARVIS</div>
        <div style="font-size:0.82rem; color:#64748b; line-height:1.6;">
          Choose a free AI provider below — no credit card needed for the free options.
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        meta = PROVIDERS[selected_provider]

        # Free tier instructions
        instructions = {
            "gemini":     ("🟢 COMPLETELY FREE", "aistudio.google.com", "Click 'Get API Key' → Create → Copy the AIza... key"),
            "groq":       ("🟡 COMPLETELY FREE", "console.groq.com",    "Sign up → API Keys → Create → Copy the gsk_... key"),
            "openrouter": ("🟣 FREE MODELS AVAILABLE", "openrouter.ai", "Sign up → Keys → Create key (free models need no credits)"),
            "claude":     ("💰 PAID", "console.anthropic.com", "Requires credits. Add payment method first."),
            "openai":     ("💰 PAID", "platform.openai.com/api-keys", "Requires credits. Add payment method first."),
        }
        badge, site, tip = instructions[selected_provider]
        st.markdown(f"""
        <div class="j-card" style="margin-bottom:0.8rem; border-color:{meta['color']}44;">
          <div style="font-size:0.72rem; font-weight:600; color:{meta['color']}; margin-bottom:4px;">{badge}</div>
          <div style="font-size:0.8rem; color:#94a3b8; margin-bottom:4px;">
            Get your free key at: <span style="color:#60a5fa;">{site}</span>
          </div>
          <div style="font-size:0.76rem; color:#64748b;">{tip}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("⚡ Initialise JARVIS →", use_container_width=True):
            ...
            if cleaned:
                ...
                st.session_state[key_map[selected_provider]] = cleaned
                st.session_state.ai_provider = selected_provider
                st.session_state.api_key_set = True
                st.rerun()
            else:
                st.error(f"Please paste your API key for {meta['label'].split(' —')[0]}.")
...
Format with clear sections. Keep under 300 words."""
            ...
            t0 = time.time()
            briefing, err = ai_complete(system="", user=prompt, max_tokens=600)
            if err:
                st.error(err)
                return
...
def render_chat():
    ...
    if send and user_msg:
        ...
        with st.spinner("JARVIS is thinking..."):
            ...
            t0 = time.time()
            reply, err = ai_complete(system=JARVIS_SYSTEM, user="", max_tokens=800, messages=msgs)
            if err:
...
def render_thought_dump():
    ...
    if dump_btn:
        ...
        with st.spinner("JARVIS is structuring your thoughts..."):
            t0 = time.time()
            raw, err = ai_complete(system=TD_SYSTEM, user=f"Brain dump:\n\n{dump_text}", max_tokens=1000)
            if err:
                st.error(err)
                return
            duration = time.time() - t0
...
Be specific and JARVIS-sharp. Max 120 words."""
                t0 = time.time()
                analysis, err = ai_complete(system="", user=prompt, max_tokens=300)
                if err:
                    st.error(err)
                    return
...
# Auto-detect any provider key from env / secrets
if not st.session_state.api_key_set:
    ...
    for env_name, prov, sess_key in env_checks:
        val = os.getenv(env_name, "")
        if not val:
            try: val = st.secrets.get(env_name, "")
            except: pass
        if val:
            st.session_state[sess_key] = val.strip()
